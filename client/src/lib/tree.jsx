...
TreeTable extends Component {
  constructor(props) {{{
      ...
      }this.loadData = wrapWithAsyncErrorHandler(this, this.loadData);}this.onSelectionChanged = wrapWithAsyncErrorHandler(this, this.onSelectionChanged);}
  ...
  componentDidMount() {
    ...
    createNodeFn = (event, data) => {
      ...
      if (this.props.actions) {
        const linksContainer = jQuery(`<span class="${"actionLinks"}"/>`);
    ...
    const treeOpts = {
      ...
      activate: this.selectMode === TreeSelectMode.SINGLE ? (...args) => this.onActivate(...args) : null,
      deactivate: this.selectMode === TreeSelectMode.SINGLE ? (...args) => this.onActivate(...args) : null,
      select: this.selectMode === TreeSelectMode.MULTI ? (...args) => this.onSelect(...args) : null
    };
